	...
	w.fieldEditor = {
		...
		submit: function() {
			...
			var methods = {
				...
				duplicateFieldName: function(row, allFieldNames) {
			...
			fieldEditor.table.find("tbody").each(function(){
				...
				var errorsCount = 0;
				var errors = [];
				var allFieldNames = Array.prototype.slice.call(fieldEditor.allFieldNames);

				$self.prevAll("tbody").each(function() {
					allFieldNames.push($(this).find(":input[name*=field_name]").val());
				});

				$.each(methods, function(lang, func) {
					if (func($self, allFieldNames) === true) {
						totalErrorsCount++;
						errorsCount++;
				...
				function(data){
					...
					$("#modal").html(data).dialog({
						...
						open: function(element, ui){
							var settingsJson = fieldSettingsInput.val(),
								settings = null,
							    cache = [],
							    arrayCache = {},
							    lastIndex;
							...
							setTimeout(function() {
								fieldEditor.callback(fieldEditor.modalOpenCallbacks, row);
							}, 50);
						},
		...
		callback: function(callbacks, row) {
				var fieldType = $(row).find(".field_type").val();
				var args = Array.prototype.slice.call(arguments, 2);
				args.unshift($(row));
				if (callbacks[fieldType] !== undefined && typeof callbacks[fieldType].callback === "function") {
					args.push(callbacks[fieldType].settings);
					callbacks[fieldType].callback.apply(row, args);
				}	
		},
		cloneField: function(fieldId, callback) {
			$.post(
				...
				function(data){
					...
					fieldEditor.callback(fieldEditor.rowCallbacks, row);
				},
				"json"
			);
		},
		initFieldSettings: function(row, form) {
			...
			if ( ! fieldSettingsInput.val()){
				fieldEditor.callback(fieldEditor.beforeSerializeCallbacks, row, form);
		...
		init: function(options) {
			$("body").addClass("field_editor");
			$.extend(this, options);
			fieldEditor.table = $("table.field_editor");
			fieldEditor.form = $("#field_editor");
			$("#clone_field a.submit").bind("click", function(e) {
				var $self = $(this);
				var buttonText = $self.text();
				var value = $("#clone_field select").val();
				e.preventDefault();
				if (value) {
					$self.html(fieldEditor.lang.loading);
					fieldEditor.cloneField(value, function() {
			...
			fieldEditor.table.find("tbody").find("tr:first").each(function() {
				var $self = $(this);
				var $form = $("form#field_type_table_"+$self.data("id"));
				fieldEditor.initFieldSettings($self, $form);
				$form.remove();
			});
