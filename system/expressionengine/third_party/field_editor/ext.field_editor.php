...
class Field_editor_ext
{
	...
	public $docs_url = 'https://devot-ee.com/add-ons/field-editor';
	...
	public function publish_form_channel_preferences($row)
	{
		...
		if (
			...
			(
				$this->EE->cp->allowed_group('can_access_addons', 'can_access_modules') &&
				isset($modules['field_editor']) &&
				! empty($this->EE->session->userdata['assigned_modules'][$modules['field_editor']['module_id']])
			)
		)
		{
			$this->EE->cp->set_right_nav(array(
	       			'field_editor_edit_field_group' => BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=field_editor'.AMP.'method=edit'.AMP.'group_id='.$row['field_group'],
	       		));
		}
