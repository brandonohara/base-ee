...
class Field_editor_mcp
{
	...
	protected $fieldtype_modal_open_js_callbacks = array(
		'grid' => 'function(row, settings) {
			$("#grid_col_settings_container").width(
				$("#modal").width() - $("#grid_col_settings_labels").width()
			);
		}',
		'assets' => 'function(row, settings) {
			$("#modal").find(".assets-options-list .assets-checkbox:first input:checkbox").prop("checked", true);
		}',
	);

	protected $fieldtype_before_serialize_js_callbacks = array(
		'grid' => 'function(row, form, settings) {
			form.find("#grid_col_settings_elements").remove();
		}',
	);
	...
	private function set_page_title($title = FALSE)
	{
        ...
        //$this->EE->cp->set_variable is deprecated as of 2.6;
	...
	public function new_group()
	{
		...
		{
			$this->EE->view->cp_page_title = lang('create_new_field_group');
		}
	...
	public function delete_confirm()
	{
		...
		$this->EE->cp->add_to_head('<link rel="stylesheet" href="'.URL_THIRD_THEMES.'field_editor/css/field_editor.css" type="text/css" media="screen" />');
		...
		{
			$this->EE->view->cp_page_title = lang('field_editor_module_name');
		}
	...
	public function edit($errors = FALSE, $existing_fields = FALSE)
	{
		...
		$this->EE->load->model(array('site_model', 'field_model'));
		...
		{
			$this->EE->view->cp_page_title = lang('field_editor_edit_group');
		}
		 ...
		 * get all fields
		 */
		$query = $this->EE->db->select('field_groups.group_name, channel_fields.*')
					->join('field_groups', 'channel_fields.group_id = field_groups.group_id')
					->order_by('channel_fields.site_id, channel_fields.group_id, channel_fields.field_order', 'asc')
					->get('channel_fields');
		...
		{
			$optgroup = $row['group_name'];
			
			if ($msm && isset($sites[$row['site_id']]))
			{
				$optgroup .= ' ('.$sites[$row['site_id']].')';
			}
			
			if ( ! isset($clone_fields[$optgroup]))
			{
				$clone_fields[$optgroup] = array();
			}
			
			$clone_fields[$optgroup][$row['field_id']] = $row['field_label'];
		...
		foreach ($current_fields as $index => $field)
		{
			...
			//this will get filled later via ajax
			$field['field_type_settings'] = '';
		...
		$blank_vars['field_type_settings'] = json_encode(array(
			array(
				 'name' => 'field_maxl',
				 'value' => '128',
			),
			...
			array(
				 'name' => 'text_field_show_fmt',
				 'value' => 'n',
			),
			array(
				 'name' => 'text_field_text_direction',
				 'value' => 'ltr',
			),
			array(
				 'name' => 'text_field_content_type',
				 'value' => 'all',
			),
			array(
				 'name' => 'text_field_show_smileys',
				 'value' => 'n',
			),
			array(
				 'name' => 'text_field_show_glossary',
				 'value' => 'n',
			),
			array(
				 'name' => 'text_field_show_spellcheck',
				 'value' => 'n',
			),
			array(
				 'name' => 'text_field_show_file_selector',
				 'value' => 'n',
			),
		));
		...
		$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'field_editor/js/jquery.settingsTable.js?'.filemtime(PATH_THIRD_THEMES.'field_editor/js/jquery.settingsTable.js').'"></script>');
		
		$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.URL_THIRD_THEMES.'field_editor/js/fieldEditor.js?'.filemtime(PATH_THIRD_THEMES.'field_editor/js/fieldEditor.js').'"></script>');

		$callbacks = array();

		$callbackTypes = array('row', 'modal_open', 'before_serialize');

		foreach ($callbackTypes as $callbackType)
		{
			$callbackArray = array();

			foreach ($this->{'fieldtype_'.$callbackType.'_js_callbacks'} as $field_type => $callback)
			{
				...
				$callbackArray[] = sprintf("%s: {\n\t\t\t\"callback\": %s,\n\t\t\t\"settings\": %s\n\t\t}", json_encode($field_type), $callback, json_encode($field_settings));
			}
		...
		$this->EE->javascript->output('
			fieldEditor.init($.extend('.json_encode(array(
				'groupId' => $group_id,
				...
				'loadingImg' => img(array(
					'src' => PATH_CP_GBL_IMG.'loadingAnimation.gif',
			...
			)).', {"rowCallbacks":'.$callbacks['row'].'}, {"modalOpenCallbacks":'.$callbacks['modal_open'].'}, {"beforeSerializeCallbacks":'.$callbacks['before_serialize'].'}));
			fieldEditor.table.settingsTable({
				...
				deleteConfirm: '.json_encode(lang('field_editor_delete_field_confirm')).',
			...
			fieldEditor.table.find("tbody tr").each(function() {
				fieldEditor.callback(fieldEditor.rowCallbacks, this);
			});
		');
	...
	{
		if ( ! $this->EE->input->post('has_all_inputs'))
		{
			if (empty($_POST))
			{
				show_error('$_POST is empty, could not save.');
			}
			else
			{
				show_error('Did not recieve all form inputs. Please try increasing your PHP <code>max_input_vars</code> and/or your <code>post_max_size</code>. If you are using Suhosin, you should also check the <code>suhosin.post.max_array_depth</code>, <code>suhosin.post.max_array_index_length, <code>suhosin.post.max_name_length</code>, <code>suhosin.post.max_totalname_length</code>, <code>suhosin.post.max_value_length</code>, <code>suhosin.post.max_vars</code> configurations.');
			}
		}
		...
		if (is_array($this->EE->input->post('delete_fields')))
		{
			...
			foreach ($query->result() as $row)
			{
				array_push($channels, $row->channel_id);
			}
			...
			foreach ($this->EE->input->post('delete_fields') as $field_id)
			{
				if ($channels)
				{
					$this->EE->layout->delete_layout_fields($field_id, $channels);
				}
	...
	public function field_settings()
	{
		...
		/**
		 * Load celltype JS/CSS when viewing Matrix settings
		 */
		/** This is no longer fully necessary as of EE 2.10.1 and/or Matrix 2.5.10, not sure which **/
		if ($field_type === 'matrix')
		{
			foreach ($field_vars['field_type_tables'] as $ftype => $fsettings)
			{
				/*
				if (isset($this->per_field_head[$ftype]))
				{
					foreach ($this->per_field_head[$ftype] as $row)
					{
						$vars['content'] = $row.$vars['content'];
					}
				}
				*/
				
				if (isset($this->per_field_foot[$ftype]))
				{
					foreach ($this->per_field_foot[$ftype] as $row)
					{
						// matrix scripts should already be loaded
						if (stripos($row, 'matrix') === FALSE)
						{
							$vars['content'] .= $row;
						}
					}
				...
				/*
				if (isset($this->per_field_code[$ftype]))
				{
					$vars['content'] .= $this->EE->javascript->inline('jQuery(document).ready(function($) {
						'.implode("\n", $this->per_field_code[$ftype]).'
					});');
				}
				*/
			}
	...
	public function clone_field()
	{
		...
		$this->EE->lang->load('admin_content');
		
		$field_id = $this->EE->input->get_post('field_id');
		$rowid = $this->EE->input->get_post('rowid');
		
		if ( ! $field_id || $rowid === FALSE)
		{
		...
		$field = array_merge($this->field_edit_vars($query->row('group_id'), $field_id), $query->row_array());
		...
		switch ($field['field_type'])
		{
			...
			case 'grid':
				...
				if (preg_match_all('/col_id_(\d+)/', $result['form'], $matches))
				{
					...
					foreach ($existing_cols as $old_col_id => $new_col_id)
					{
						$result['form'] = str_replace('col_id_'.$old_col_id, 'new_'.$new_col_id, $result['form']);
					}
				}
	...
	private function field_edit_vars($group_id, $field_id = FALSE, $field_type = FALSE)
	{
		...
		//get the group's site_id
		$query = $this->EE->db->select('site_id')
					    ->where('group_id', $group_id)
					    ->get('field_groups');
		
		$this->EE->config->set_item('site_id', $query->row('site_id'));
		...
		if ($field_type)
		{
			...
			$this->EE->javascript->js->mcp = $this;
			$this->EE->cp->mcp = $this;
			...
			if (isset($this->per_field_head[$field_type]))
			{
				foreach ($this->per_field_head[$field_type] as $row)
				{
					$vars['pre'] .= $row;
				}
			}
			...
			if (isset($this->per_field_foot[$field_type]))
			{
				foreach ($this->per_field_foot[$field_type] as $row)
				{
					$vars['post'] .= $row;
				}
			}
			
			if (isset($this->per_field_code[$field_type]))
			{
				$vars['post'] .= $this->EE->javascript->inline('jQuery(document).ready(function($) {
					'.implode("\n", $this->per_field_code[$field_type]).'
				});');
			}
		...
		$this->EE->config->set_item('site_id', $cache['site_id']);
	...
	private function update_field($field_data, $keep_existing_settings = FALSE)
	{
		...
		{
			// for CT's select or checkbox field type, we're stripping slashes
			if (($field_data['field_type'] == "select" || $field_data['field_type'] == "checkbox") && isset($field_data['field_list_items']))
			{
	 			$field_data['field_list_items'] = stripslashes($field_data['field_list_items']); 
			}
			elseif (isset($field_data['field_list_items']))
			{
	 			$field_data['field_list_items'] = stripslashes($field_data['field_list_items']); 
			}
		}
	...
	public function import_upload()
	{
		...
		{
			$this->EE->view->cp_page_title = lang('field_editor_import_group');
		}
	...
	public function backtrace_fieldtype()
	{
...
class Field_editor_Cp extends Cp
{
	...
	public function add_to_head($data)
	{
		...
		if ($fieldtype = $this->mcp->backtrace_fieldtype())
		{
			$this->mcp->per_field_head[$fieldtype][] = $data;
		}
	}
	...
	public function add_to_foot($data)
	{
		...
		if ($fieldtype = $this->mcp->backtrace_fieldtype())
		{
			$this->mcp->per_field_foot[$fieldtype][] = $data;
		}
	}
}
...
class Field_editor_Jquery extends CI_Jquery
{
	...
	public function _output($data)
	{
		...
		if ($fieldtype = $this->mcp->backtrace_fieldtype())
		{
			$this->mcp->per_field_code[$fieldtype][] = $data;
		}
	}
}
