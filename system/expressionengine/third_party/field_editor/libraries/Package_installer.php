...
class Package_installer
{
	...
	private $format = 'xml';
	// intended to be used in case an array needs to be passed by reference. this is not intended to be set early
	// but only ad-hoc as necessary. Object properties can be passed by reference these days... arrays shouldnt' be
	private $local_field_data = array(); 
	...
	{
		static $installed_fieldtypes;
		
		if (is_null($installed_fieldtypes))
		{
			$query = $this->EE->db->select('name')
					      ->get('fieldtypes');
			
			foreach ($query->result() as $row)
			{
				$installed_fieldtypes[] = $row->name;
			}
		...
		// passing general arrays as references became deprectaed. So I converted the array to an object property. problem solved. 
		$this->local_field_data = $field_data; 
		
		$original_name =$this->local_field_data['field_name'];
		
		if ( ! in_array($this->local_field_data['field_type'], $installed_fieldtypes))
		{
			return $this->log_error('fieldtype_not_installed', $this->local_field_data['field_type'], $original_name);
		}
		
		$this->local_field_data['site_id'] = $this->EE->config->item('site_id');
		
		$this->local_field_data['field_name'] = $this->rename('channel_fields', $original_name, 'field_name');

		if ($this->local_field_data['field_name'] === FALSE)
		{
			return $this->log_error('field_exists', $original_name);
		}

		$column_type = isset($this->local_field_data['column_type']) ? $this->local_field_data['column_type'] : NULL;

		unset($this->local_field_data['column_type']);

		$field_id = $this->insert('channel_fields', $this->local_field_data);
		
		if ($column_type)
		{
			$this->EE->dbforge->add_column('channel_data', array('field_id_'.$field_id => $this->column_type_to_options($column_type)));
		}
		elseif (@$this->local_field_data['field_type'] == 'date' || @$this->local_field_data['field_type'] == 'rel')
		{
		...
		if (@$this->local_field_data['field_type'] == 'date')
		{
			$this->EE->dbforge->add_column('channel_data', array('field_dt_'.$field_id => array('type' => 'varchar(50)')));
		}
		
		if (method_exists($this, 'create_field_'.$this->local_field_data['field_type']) && isset($this->local_field_data[$this->local_field_data['field_type']]) && $field_type_data = @unserialize(base64_decode($this->local_field_data[$this->local_field_data['field_type']])))
		{
			call_user_func(array(&$this, 'create_field_'.$field_data['field_type']), $field_id, $field_type_data, $field_data);
		}
		...
		$this->log_install('field', $this->local_field_data['field_name']);
		
		// reset back to zero. 
		$this->local_field_data = array(); 
	}

	/**
	 * Convert a column type definitino to a DB Forge compatible array of column options
	 * @param  string $column_type
	 * @return array
	 */
	private function column_type_to_options($column_type)
	{
		$options = array();

		if (preg_match('/^(.+) AUTO_INCREMENT$/', $column_type, $match))
		{
			$column_type = $match[1];
			$options['auto_increment'] = TRUE;
		}

		if (preg_match('/^(.+) DEFAULT (.*?)$/', $column_type, $match))
		{
			$column_type = $match[1];
			$default = $match[2];
			...
			{
				$options['default'] = $match[1];
			}
		}

		if (preg_match('/^(.+) NOT NULL$/', $column_type, $match))
		{
			$column_type = $match[1];
			$options['null'] = FALSE;
		}

		if (preg_match('/^(.+) unsigned$/', $column_type, $match))
		{
			$column_type = $match[1];
			$options['unsigned'] = TRUE;
		}

		if (preg_match('/^(.+)\((\d+)\)$/', $column_type, $match))
		{
			$column_type = $match[1];
			$options['constraint'] = $match[2];
		}
	...
	private function create_field_grid($field_id, $field_type_data, $field_data)
	{
		$col_ids = array();

		$this->EE->load->model('grid_model');

		$this->EE->grid_model->create_field($field_id, 'channel');

		foreach ($field_type_data as $row)
		{
			$row['field_id'] = $field_id;

			unset($row['col_id']);

			$column_options = array('type' => 'text', 'null' => FALSE);

			if (isset($row['column_type']))
			{
				$column_options = $this->column_type_to_options($row['column_type']);

				unset($row['column_type']);
			}

			$col_id = $this->insert('grid_columns', $row);

			$col_ids[] = $col_id;

			$this->EE->dbforge->add_column('channel_grid_field_'.$field_id, array('col_id_'.$col_id => $column_options));
		}
		
		$field_settings = unserialize(base64_decode($field_data['field_settings']));
		
		$field_settings['col_ids'] = array_values($col_ids);
		
		$field_data['field_settings'] = base64_encode(serialize($field_settings));
	}
	// previously $field_data was being passed as a refernce. Instead of passing by reference, you should pass an object property. 
	// I changed this function because passing arrays by reference is deprecated, as is passing anything as a reference in a user call function
	private function create_field_matrix($field_id, $field_type_data, $field_data)
	{
		$col_ids = array();
		
		foreach ($field_type_data as $row)
		{
			$row['site_id'] = $this->EE->config->item('site_id');
			...
			$col_id = $this->insert('matrix_cols', $row);

			$col_ids[] = $col_id;

			$this->EE->dbforge->add_column('matrix_data', array('col_id_'.$col_id => $column_options));
		}
		
		$field_settings = unserialize(base64_decode($field_data['field_settings']));
		
		$field_settings['col_ids'] = $col_ids;
	...
	private function create_template($template, $group_id, $group_name)
	{
		...
		if (isset($template_data['template_type']))
		{
			...
			$ext = $this->EE->api_template_structure->file_extensions($template_data['template_type']);
		}
		
		$template_file = $this->template_path.$group_name.'.group'.DIRECTORY_SEPARATOR.$template_data['template_name'].$ext;
